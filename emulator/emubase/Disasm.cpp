...
// Disasm.cpp
// Disassembler for 1801VM1 processor
...
bool Disasm_CheckForJump(const uint16_t* memory, int* pDelta)
{
    ...
    // SOB
    if ((instr & ~static_cast<uint16_t>(0777)) == PI_SOB)
    {
    ...
    {
        *pDelta = (static_cast<short>(memory[1]) + 4) / 2;
        return true;
    }
...
void Disasm_RegisterHint(const CProcessor * pProc, const CMotherboard * pBoard,
...
void Disasm_InstructionHint(const uint16_t* memory, const CProcessor * pProc, const CMotherboard * pBoard,
        ...
        int srcreg, int srcmod, int dstreg, int dstmod)
{
    ...
    if (srcreg >= 0)
    {
        ...
        else
        {
            ...
            Disasm_RegisterHint(pProc, pBoard, srchint1, srchint2, srcreg, srcmod, byteword, indexval);
        }
    }
    if (dstreg >= 0)
    {
        ...
        else
        {
            ...
            Disasm_RegisterHint(pProc, pBoard, dsthint1, dsthint2, dstreg, dstmod, byteword, indexval);
        }
    }
        ...
        LPTSTR buffer, LPTSTR buffer2)
{
    ...
    *buffer = 0;  *buffer2 = 0;
        ...
        (instr & ~(uint16_t)0007777) == PI_ADD || (instr & ~(uint16_t)0007777) == PI_SUB)
    {
        ...
        Disasm_InstructionHint(memory, pProc, pBoard, buffer, buffer2, srcreg, srcmod, dstreg, dstmod);
    }
        ...
        (instr & ~(uint16_t)0777) == PI_XOR)
    {
        ...
        int dstreg = instr & 7;
        int dstmod = (instr >> 3) & 7;
        Disasm_InstructionHint(memory, pProc, pBoard, buffer, buffer2, srcreg, 0, dstreg, dstmod);
    }
        ...
        (instr & ~(uint16_t)077) == PI_MTPS || (instr & ~(uint16_t)077) == PI_MFPS)
    {
        int dstreg = instr & 7;
        int dstmod = (instr >> 3) & 7;
        Disasm_InstructionHint(memory, pProc, pBoard, buffer, buffer2, -1, -1, dstreg, dstmod);
    }
        ...
        (instr & ~(uint16_t)0100077) == PI_ROR || (instr & ~(uint16_t)0100077) == PI_ROL)
    {
        ...
        if (dstreg != 7)
        {
            ...
            Disasm_InstructionHint(memory, pProc, pBoard, tempbuf, buffer2, -1, -1, dstreg, dstmod);
            uint16_t psw = pProc->GetPSW();
